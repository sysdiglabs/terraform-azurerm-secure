...
locals {
    tenant_subscription_ids = split(",", data.external.subscriptions.result["subscription_ids"])
}

data "azurerm_subscription" "tenant_subs" {
  count = length(local.tenant_subscription_ids)
  subscription_id = local.tenant_subscription_ids[count.index]
}
...
resource "azurerm_monitor_diagnostic_setting" "sysdig_diagnostic_setting_sub" {
    ...
    target_resource_id         = data.azurerm_subscription.tenant_subs[count.index].id
